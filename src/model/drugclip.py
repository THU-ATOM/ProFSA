...
from src.dataset.profsa2 import AtomTokenizer

from .components.unimol import NonLinearHead, UniMolModel
...
class DrugCLIP2(DrugCLIP):
    def __init__(self, cfg: Dict[str, Any]):
        ...
        self.mol_dict, _ = self.load_dictionary()

        self.mol_model = UniMolModel(
            cfg.mol, self.mol_dict.pad(), len(self.mol_dict)
        )
        self.pocket_model = UniMolModel(
            cfg.pocket, self.tokenizer.pad_idx, len(self.tokenizer)
        )
